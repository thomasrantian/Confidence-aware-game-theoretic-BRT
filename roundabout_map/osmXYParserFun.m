...
function varargout = osmXYParserFun(filename, faded, IS_SHOW)
if nargin == 0
    ...
    if IS_SHOW
        if faded
            ...
            for id = 1 : 1 : length(Lines)
                ...
                xy = tmpInfo.xy;
                if strcmpi(tmpInfo.type, 'curb')
                    p1 = plot(xy(1, :), xy(2, :), 'k-','DisplayName',"Road boundary",'LineWidth',2);
                    p1.Color(4) = 0.4;
                    p1.Annotation.LegendInformation.IconDisplayStyle = 'off';
                end
                if strcmpi(tmpInfo.type, 'lane')
                    p2 = plot(xy(1, :), xy(2, :), 'k-','DisplayName',"Road boundary",'LineWidth',2);
                    p2.Color(4) = 0.4;
                    p2.Annotation.LegendInformation.IconDisplayStyle = 'off';
                end
                if strcmpi(tmpInfo.type, 'stop')
                    p3 = plot(xy(1, :), xy(2, :), 'k-','DisplayName',"Road boundary",'LineWidth',2);
                    p3.Color(4) = 0.4;
